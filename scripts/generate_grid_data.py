...
ghana_regions = {
    "Greater Accra": [
        ("Achimota", 5.614, -0.224),
        ("Tema", 5.669, -0.017),
        ("Mallam", 5.560, -0.298),
        ("Legon", 5.650, -0.186),
        ("Kaneshie", 5.560, -0.238),
        ("Aboadze Junction", 5.590, -0.150),
    ],
    "Ashanti": [
        ("Kumasi Central", 6.688, -1.624),
        ("Ejisu", 6.719, -1.463),
        ("Obuasi", 6.202, -1.663),
        ("Mampong", 7.062, -1.400),
        ("Konongo", 6.618, -1.219),
    ...
    "Western": [
        ("Takoradi", 4.895, -1.759),
        ("Aboadze", 4.985, -1.766),
        ("Tarkwa", 5.301, -1.994),
        ("Axim", 4.867, -2.241),
    ],
    "Central": [
        ("Cape Coast", 5.106, -1.246),
        ("Winneba", 5.352, -0.622),
        ("Kasoa", 5.533, -0.416),
    ...
    "Bono": [
        ("Sunyani", 7.339, -2.326),
        ("Techiman", 7.590, -1.938),
        ("Berekum", 7.453, -2.585),
    ],
    "Northern": [
        ("Tamale", 9.403, -0.842),
        ("Yendi", 9.442, -0.011),
        ("Savelugu", 9.625, -0.826),
    ],
    "Upper East": [
        ("Bolgatanga", 10.787, -0.851),
        ("Bawku", 11.058, -0.243),
    ],
    "Upper West": [
        ("Wa", 10.061, -2.501),
...
for _region, places in ghana_regions.items():
    for name, lat, lon in places:
        ...
        sub_type = (
            "Transmission"
            if voltage >= 161
            else ("Bulk Supply Point" if voltage == 69 else "Distribution")
        )
        capacity = round(
            (
                random.uniform(15, 400)
                if sub_type != "Distribution"
                else random.uniform(5, 60)
            ),
            1,
        )
    ...
    for i, a in enumerate(ids_in_region):
        for b in ids_in_region[i + 1 :]:
            if random.random() < 0.55:
                ...
                dist = round(
                    haversine_km(sub_a[5], sub_a[6], sub_b[5], sub_b[6])
                    * random.uniform(1.05, 1.3),
                    1,
                )
