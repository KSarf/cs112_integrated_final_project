...
def main() -> None:
    ...
    # 1. Load Datasets
    substations_file = PROCESSED_DIR / "substations_clean.csv"
    lines_file = PROCESSED_DIR / "lines_clean.csv"
    n1_file = PROCESSED_DIR / "n1_node_results.csv"

    if substations_file.exists() and lines_file.exists():
        substations = pd.read_csv(substations_file)
        lines = pd.read_csv(lines_file)
    else:
        st.warning(
            "Cleaned data was not found. Using raw generated datasets. "
            "Run grid_analysis/run_minimum_analysis.py to create processed results."
        )
    ...
    if len(filtered_substations) > 0:
        connectivity_ratio = len(filtered_lines) / len(filtered_substations)
    else:
        connectivity_ratio = 0

    col3.metric(
        "Connectivity Ratio",
        f"{connectivity_ratio:.2f} / node",
    )

    if n1_file.exists():
        n1_results = pd.read_csv(n1_file)

        if len(n1_results) > 0:
            resilient_count = (~n1_results["Fragmented"]).sum()

            resilience_percent = (resilient_count / len(n1_results)) * 100
        else:
            resilience_percent = 0.0

        col4.metric(
            "N-1 Node Resilience",
            f"{resilience_percent:.1f}%",
        )
    else:
        col4.metric(
            "N-1 Node Resilience",
    ...
    if view == "Overview":
        ...
        st.write(
            "Summary breakdown of grid infrastructure and regional capacity distribution."
        )
        ...
        with c1:
            fig_bar = px.histogram(
                ...
                x="Region",
                color="Region",
                title="Substation Count by Region",
            )
        ...
        with c2:
            voltage_col = (
                "Voltage (kV)"
                if "Voltage (kV)" in filtered_lines.columns
                else filtered_lines.columns[2]
            )
            fig_pie = px.pie(
                filtered_lines,
                names=voltage_col,
                title="Transmission Line Voltage Classes",
                hole=0.4,
            )
    ...
    # --- TAB 2: NETWORK (Force-Directed Graph) ---
        ...
        st.caption(
            "Force-directed topology layout. Hover over nodes to inspect details."
        )
        ...
        pos = nx.spring_layout(graph, k=0.35, iterations=50, seed=42)
        ...
        hover_texts = [
            f"<b>{graph.nodes[n].get('name', n)}</b><br>Region: {graph.nodes[n].get('region', 'N/A')}<br>Connections: {graph.degree(n)}"
        ...
        node_trace = go.Scatter(
            ...
            marker=dict(size=14, color="#00d2ff", line=dict(width=2, color="#ffffff")),
        )
    ...
    # --- TAB 3: GEOGRAPHY (Geospatial Map) ---
        ...
        fig_map = px.scatter_map(
            ...
            color="Region",
            hover_name=(
                "Name" if "Name" in filtered_substations.columns else "Substation ID"
            ),
            hover_data=(
                ["Voltage (kV)", "Country"]
                if "Voltage (kV)" in filtered_substations.columns
                and "Country" in filtered_substations.columns
                else None
            ),
            zoom=5.5,
            height=600,
    ...
    # =========================================================================
        ...
        st.write(
            "Analysis of substation operational capacities, network centrality, and N-1 resilience."
        )
        ...
        deg_df = pd.DataFrame(
            list(degree_dict.items()), columns=["Substation ID", "Connections (Degree)"]
        )
        ...
        if "Name" in filtered_substations.columns:
            name_map = dict(
                zip(
                    filtered_substations["Substation ID"],
                    filtered_substations["Name"],
                    strict=True,
                )
            )
        else:
        ...
        deg_df["Substation Name"] = (
            deg_df["Substation ID"].map(name_map).fillna(deg_df["Substation ID"])
        )

        # Pick the top 12 most-connected substations (critical hubs)
        top_hubs = deg_df.sort_values(by="Connections (Degree)", ascending=True).tail(
            12
        )
        ...
        st.caption(
            "Simulate taking a major substation offline to check if the national network fragments."
        )
        ...
        if post_components == initial_components:
            col3.success(
                "Resilient: Grid remains fully connected with 0 fragmentation!"
            )
        else:
            col3.error(
                f"Alert: Grid split into {post_components} separate isolated parts!"
            )
    ...
    # --- TAB 5: SEARCH & COMPARISON ---
        ...
        selected_sub = st.selectbox(
            "Select a Substation to Inspect", filtered_substations[name_col].unique()
        )

        sub_record = filtered_substations[
            filtered_substations[name_col] == selected_sub
        ].iloc[0]
        ...
        with c2:
            ...
            connected_lines = lines[
                (lines["Source Substation ID"] == sub_id)
                | (lines["Destination Substation ID"] == sub_id)
            ]
